...
test('친구 소개 화면은 내 친구는 솔로 브랜드와 신랑·신부가 준비한 프로필만 표시한다', () => {
  const html = fs.readFileSync(require.resolve('../index.html'), 'utf8');
  ...
  assert.doesNotMatch(html, /loadSoloProfiles\(\);/);
  ...
  assert.doesNotMatch(html, /SOLO_APPLICATIONS_KEY|soloServerProfiles|soloSessionProfiles/);
  assert.doesNotMatch(html, /function (?:getStoredSoloApplications|saveSoloApplication|loadSoloProfiles|normalizeSoloPublicProfile)\s*\(/);
