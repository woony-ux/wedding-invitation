...
function createButton({
  name = '함상구',
  bank = '국민은행 246-21-0264-767'
} = {}) {
  const classes = new Set();
  const accountRow = {
    querySelector: selector => ({
      '.acc-name': { textContent: name },
      '.acc-bank': { textContent: bank }
    })[selector]
  };
  return {
    textContent: '복사',
    closest: selector => selector === '.acc-row' ? accountRow : null,
    classList: {
...
test('최신 복사 방식이 성공하면 복사됨을 표시한다', async () => {
  ...
  await context.copyAcc(button);
  await new Promise(resolve => setImmediate(resolve));

  assert.equal(copiedText, '국민은행 246-21-0264-767 예금주 함상구');
...
test('모든 복사 방식이 실패하면 성공으로 표시하지 않고 직접 복사를 안내한다', async () => {
  ...
  assert.deepEqual(modalCalls, [[
    'icon-message',
    '계좌번호를 복사하지 못했어요',
    '국민은행 246-21-0264-767 예금주 함상구\n길게 눌러 직접 복사해 주세요.'
  ]]);
  assert.equal(bodyChildren.length, 0);
});

test('모든 계좌 복사 버튼은 화면의 은행명·계좌번호·예금주를 사용한다', () => {
  const buttons = [...html.matchAll(/onclick="copyAcc\(this\)"/g)];

  assert.equal(buttons.length, 5);
  assert.equal(/copyAcc\(this,'[^']+'\)/.test(html), false);
});
